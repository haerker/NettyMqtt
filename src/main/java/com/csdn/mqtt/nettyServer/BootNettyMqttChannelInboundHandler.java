        ...
        if (null != msg) {
            ...
            switch (mqttFixedHeader.messageType()) {
                case PUBLISH:        //	客户端发布消息
                    ...
                    // 检查是否有订阅该主题的客户端
                    for (Map.Entry<ChannelHandlerContext, List<String>> entry : subTopicMap.entrySet()) {
                        for (String oneOfTopic : entry.getValue()) {
                            if (oneOfTopic.equals(pubTopic)) {
                                // 如果客户端订阅了这个主题，将消息发送给他们
                                BootNettyMqttMsgBack.publishToClient(entry.getKey(), publishMessage);
                            }
                        }
                    }
                ...
                case SUBSCRIBE:        //	客户端订阅主题
                    ...
                    for (MqttTopicSubscription subscription : topicSubscriptions) {
                        topicSubscriptName.add(subscription.topicName());
                    }
                    subTopicMap.put(ctx, topicSubscriptName);
